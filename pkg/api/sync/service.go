...
func New(db *pg.DB, sdb Repository, rbac RBAC, sec Securer) *Sync {
	return &Sync{db: db, sdb: sdb, rbac: rbac, sec: sec}
}

// Initialize initializes Sync application service with defaults
func Initialize(db *pg.DB, rbac RBAC, sec Securer) *Sync {
